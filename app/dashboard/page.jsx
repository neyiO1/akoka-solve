...
export default function Dashboard() {
  ...
  useEffect(() => {
    // In a real app, this comes from React Context / Firebase Auth
    // For now, we simulate fetching the offline DB tasks
  ...
  return (
    <div style={{ 
      ...
      <div style={{ flex: 1, display: "flex", maxWidth: "1200px", margin: "0 auto", width: "100%", gap: "24px", padding: "20px" }}>
        ...
        {/* Sidebar (Appears like bottom nav on mobile, sidebar on desktop) */}
        <div className="dashboard-sidebar" style={{ 
          ...
          {["Home", "My Tasks", "Esusu Pool", "Digital CV", "Settings"].map((item, i) => (
            <button key={i} style={{
              background: i === 0 ? "rgba(43, 108, 176, 0.2)" : "transparent",
              color: i === 0 ? "var(--blue)" : "var(--grey-light)",
              border: `1px solid ${i === 0 ? "rgba(43, 108, 176, 0.3)" : "transparent"}`,
              padding: "12px 16px",
              borderRadius: "8px",
              textAlign: "left",
              fontWeight: 600,
            ...
            onMouseLeave={(e) => e.target.style.background = i === 0 ? "rgba(43, 108, 176, 0.2)" : "transparent"}
            >
              {item}
            </button>
          ))}
